...
const MyExpense = () => {
  ...
  const MyExpenseSubmitHandler = (event) => {
    ...
    if (
      ...
      fetch(
        `https://expensetracker-dd10a-default-rtdb.firebaseio.com/expense/${replaceEmailid}.json`,
        {
          method: "POST",
          body: JSON.stringify({
            exprenseData,
  ...
  const DeleteExpenseHandler = async (id) => {
    try {
      if (window.confirm("Are you sure you want to delete this expense?")) {
        await fetch(
          `https://expensetracker-dd10a-default-rtdb.firebaseio.com/expense/${replaceEmailid}/${id}.json/`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const EditExpenseHandler = async (editexpense) => {
    ...
    try {
      const res = await fetch(
        `https://expensetracker-dd10a-default-rtdb.firebaseio.com/expense/${replaceEmailid}/${editexpense.id}.json/`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
